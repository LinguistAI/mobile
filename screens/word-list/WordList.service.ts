...
import { DictionaryResponse, IAddWord, ICreateWordList, IEditWordList, IWordListWithUserInfo, IWordListsWithUserInfo } from "./types";

export const createList = async (list: ICreateWordList) => {
    const response = await axiosSecure.post<APIResponse<IWordListWithUserInfo>>("wordbank/lists", list);
...
export const getWordMeanings = async (wordList: string[]) => {
    const response = await axiosSecure.post<APIResponse<DictionaryResponse>>("dictionary", {wordList})
    return response.data
}